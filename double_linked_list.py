class Node:
    def __init__(self, data=None, next=None, prev=None):
        self.data = data
        self.next = next
        self.prev = prev

class LinkedList:
    def __init__(self):
        self.head = None
    
    def insert_at_begining(self, data):
        current_head = self.head
        node = Node(data, next=current_head, prev=None)
        current_head.prev = node

    def insert_at_end(self, data):
        if self.head is None:
            self.head = Node(data, None, None)
            return

        itr = self.head

        while itr.next:
            itr = itr.next

        itr.next = Node(data, next=None, prev=itr)
    
    def print(self, back=False):
        if self.head is None:
            print("Linked list is empty")
            return
        itr = self.head
        llstr = ''
        if not back:
            while itr:
                llstr += str(itr.data)+' --> ' if itr.next else str(itr.data)
                itr = itr.next
        else:
            while itr.next:
                itr = itr.next
            while itr:
                llstr+= str(itr.data)+' <-- ' if itr.prev else str(itr.data)
                itr = itr.prev
        print(llstr)
        
    def get_length(self):
        count = 0
        itr = self.head
        while itr:
            count+=1
            itr = itr.next

        return count
        
    def insert_at(self, index, data):
        if index<0 or index>self.get_length():
            raise Exception("Invalid Index")

        if index==0:
            self.insert_at_begining(data)
            return

        count = 0
        itr = self.head
        while itr:
            if count == index - 1:
                node = Node(data, next=itr.next, prev=itr)
                itr.next = node
                break

            itr = itr.next
            count += 1
            
    def remove_at(self, index):
        if index<0 or index>=self.get_length():
            raise Exception("Invalid Index")

        if index==0:
            self.head = self.head.next
            return

        count = 0
        itr = self.head
        while itr:
            if count == index - 1:
                new_node = itr.next.next
                itr.next = new_node
                new_node.prev = itr
                break

            itr = itr.next
            count+=1
            
    def insert_values(self, data_list):
        self.head = None
        for data in data_list:
            self.insert_at_end(data)

    def insert_after_value(self, data_after, data_to_insert):
    # Search for first occurance of data_after value in linked list
    # Now insert data_to_insert after data_after node
        current = self.head
        while current:
            if current.data == data_after:
                next_point = current.next
                current.next = Node(data_to_insert, next=next_point, prev=current)
                break
            current = current.next

    def remove_by_value(self, data):
    # Remove first node that contains data
        index_of_data = self.get_index(data)
        if not index_of_data:
            self.print()
            raise Exception(f"{data} not found in list")
        self.remove_at(index_of_data)
    
    def get_index(self, data):
        index = 0
        current = self.head
        while current:
            if current.data == data:
                return index
            index+=1
            current = current.next

if __name__ == '__main__':
    ll = LinkedList()
    ll.insert_at_end(1)
    ll.insert_at_end(2)
    ll.insert_at_end(3)
    ll.insert_at_end(4)
    ll.insert_at_end(5)
    
    ll.print()
    ll.print(back=True)
    print(ll.get_length())
    
    ll.insert_at(3,100)
    ll.print()
    ll.remove_at(1)
    ll.print()

    ll.insert_values(["banana","mango","grapes","orange"])
    ll.print()
    ll.insert_after_value("mango","apple") # insert apple after mango
    ll.print()
    
    ll.remove_by_value("grapes")
    ll.print()
    

    
